...
if (_activated) then {
	...
	//--- Detect gun
	_weaponTypesID = _logic getvariable ["type",getnumber (configfile >> "cfgvehicles" >> typeof _planeClass >> "moduleCAStype")];
	...
	waituntil {
		...
		if ((_settedPos distance _posATL > 0) && _fireProgress == 0) then {
			_posATL = _settedPos;
			_pos = +_posATL;
			_pos set [2,(_pos select 2) + getterrainheightasl _pos];
			_dir = 0;
