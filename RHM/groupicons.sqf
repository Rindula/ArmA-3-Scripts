...
if(visibleMap) then {
    {
        if((player != _x) && (group _x == group player)) then {
    ...
    {
        {
            ...
            if(!isNil "_unit") then
            {
				...
				if (vehicle _unit != _unit) then {
					_marker setMarkerTextLocal format["||Group locator|| %1 || Fahrzeug: %2", name _unit, getText(configfile >> "CfgVehicles" >> typeOf (vehicle _unit) >> "displayName")];
				} else {
					_marker setMarkerTextLocal format["||Group locator|| %1", name _unit];
				};
